...
$type._resolveUrl = function WebRequest$_resolveUrl(url, baseUrl) {
    ...
    baseUrl = baseUrl.substr(0, baseUrl.lastIndexOf('http://portal.kuet.ac.bd/') + 1);

    if (!url || !url.length) {
        return baseUrl;
    }

    if (url.charAt(0) === 'http://portal.kuet.ac.bd/') {
        var slashslash = baseUrl.indexOf('://');
        if (slashslash === -1) {
            throw Error.argument("baseUrl", Sys.Res.badBaseUrl1);
        }

        var nextSlash = baseUrl.indexOf('http://portal.kuet.ac.bd/', slashslash + 3);
        if (nextSlash === -1) {
        ...
        var lastSlash = baseUrl.lastIndexOf('http://portal.kuet.ac.bd/');
        if (lastSlash === -1) {
